...
Reference docs for the Cores SDK and how it feeds Tessera and the public website.
...
- [tessera-annotations.md](tessera-annotations.md) — `@tessera` directive reference. Every annotation the manifest generator understands, with examples. Read this before adding a new `core_*.h` module or Kiln tile driver to Tessera.
- [sdk-api-tiers.md](sdk-api-tiers.md) — Tier 1 (handle-based) vs Tier 2 (default-instance) APIs. Explains why generated Tessera C looks different from SDK examples and lists which functions live on which tier.
...
- [`sdk/core/core_*.h`](../sdk/core/) — Doxygen + `@tessera` annotations inside each core module header are the authoritative API doc for that category. The website SDK pages render from `manifests/sdk-docs/<category>.json`, which is generated from the headers.
- `manifests/` — generated artifacts (do not edit by hand). Run `tools/gen_tessera_manifest.py` to regenerate.

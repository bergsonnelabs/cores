...
TILE_JSON = kiln/definitions/$(TILE).json
PROJECT_JSON = projects/$(PROJECT)/project.json
GEN_DIR = projects/$(PROJECT)/coregen
...
ifeq ($(TILE),$(filter $(TILE),Core-U-1-a Core-U-2-a))
  ...
  FPU         = fpv5-sp-d16
  FLOAT_ABI   = hard
  LDSCRIPT    = sdk/device/stm32wba55hg.ld
  ...
  FPU         = fpv5-sp-d16
  FLOAT_ABI   = hard
  LDSCRIPT    = sdk/device/stm32h523he.ld
...
BUILD_DIR = projects/$(PROJECT)/build
...
KILN_ENABLED ?= 0
ifeq ($(KILN_ENABLED),1)
  # Auto-detect drivers from coregen output (can still be overridden on CLI).
  # core_drivers.mk is generated by coregen alongside the headers.
  # Make will restart after coregen produces it.
  ...
  # Add specific driver sources (auto-detected from core_drivers.mk or manual)
  ifdef KILN_DRIVERS
...
$(GEN_HEADERS) $(GEN_DIR)/core_drivers.mk: $(TILE_JSON) $(wildcard $(PROJECT_JSON)) tools/coregen/coregen.py tools/coregen/templates/*.j2
	@echo "  GEN   $(TILE)"
...
.PHONY: all clean clean-all distclean flash size
...
distclean: clean-all
	rm -rf projects/*/coregen/
	rm -f projects/*/core.h
	rm -f projects/*/tiles.h

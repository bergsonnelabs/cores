...
ifeq ($(KILN_ENABLED),1)
  ...
  # tile_drivers.mk is generated by coregen alongside the headers.
  # Make will restart after coregen produces it.
  -include $(GEN_DIR)/tile_drivers.mk
  CFLAGS += -I"$(KILN_DIR)/inc" -I"$(KILN_DIR)/HAL" -I"$(KILN_DIR)/Templates"
  KILN_SOURCES = $(KILN_DIR)/HAL/tiles_hal_core.c
  # Add specific driver sources (auto-detected from tile_drivers.mk or manual)
  ifdef KILN_DRIVERS
...
$(GEN_HEADERS) $(GEN_DIR)/tile_drivers.mk: $(TILE_JSON) $(wildcard $(PROJECT_JSON)) tools/coregen/coregen.py tools/coregen/templates/*.j2
	@echo "  GEN   $(TILE)"
